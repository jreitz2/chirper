...
const FollowButton = ( {chirp, db, user} ) => {
        ...
        if (user !== null) {
            ...
            if (newUser.following === []) {
                await updateDoc(userRef, {
                    following: [{id: newFollow, name:newFollowName, photo: newFollowPhoto}],
                    followingIds: [newFollow]
                })
                if (newFollowedUser.followedBy === []) {
                    await updateDoc(followedUserRef, {
                        followedBy: [{id: user.uid, name: newUser.name, photo: newUser.photo}]
                    })
                } else {
                    await updateDoc(followedUserRef, {
                        followedBy: [...newFollowedUser.followedBy, {id: user.uid, name: newUser.name, photo: newUser.photo}]
                    })
                }
            } else if(!newUser.followingIds.includes(newFollow)) {
                await updateDoc(userRef, {
                    following: [...newUser.following, {id: newFollow, name:newFollowName, photo: newFollowPhoto}],
    ...
    return ( 
        <div>
            <img src={follow} alt="follow" onClick={handleFollow} />
        </div>
     );
}
